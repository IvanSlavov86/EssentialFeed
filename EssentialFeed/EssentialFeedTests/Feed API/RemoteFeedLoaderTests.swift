...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_deliversErrorOnNon200HHHTPResponse() {
        let (sut, client) = makeSUT()
        let samples = [199, 201, 300, 400, 401, 500]
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: failure(.invalidData), when: {
                let json = makeItemJson([])
                client.complete(withStatuCode: code, data: json, at: index)
            })
        }
    }
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        sut = nil
        client.complete(withStatuCode: 200, data: makeItemJson([]))
    ...
    private func expect(_ sut: RemoteFeedLoader, toCompleteWith expectedResult: RemoteFeedLoader.Result,
        ...
        sut.load { receivedResult in
            ...
            case let (.success(receivedItem), .success(expectedItems)):
                XCTAssertEqual(receivedItem, expectedItems, file: file, line: line)
    ...
    private class HTTPClientSpy: HTTPClient {
        ...
        func complete(with errror: Error, at index: Int = 0) {
            messages[index].completion(.failure(errror))
        }
        
        func complete(withStatuCode code: Int, data: Data, at index: Int = 0) {
